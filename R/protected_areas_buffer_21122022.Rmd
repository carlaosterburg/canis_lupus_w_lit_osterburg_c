---
title: "protected_areas_buffer_21122022"
author: "Carla"
date: "2022-12-21"
...
setwd("C:/Users/carla/Documents/Master Thesis/canis_lupus_w_lit_osterburg_c") # sets the working directory
...
data.input <- file.path(here::here("data-raw","r-raw")) # specifies where data input comes from
...
# extract the study area size and taking out the NAs 
study_area_size_data_no_nas <- study_area_size_data %>% 
  na.omit() %>%
 dplyr:: select(longitude, latitude) %>%
  tibble::rowid_to_column("id")

# calculate the radius of the buffer based on the study area size
buffer_size <- study_area_size_data %>%
  na.omit() %>%
  mutate(radius=(sqrt(study_area_size/pi) * 1000)) %>% # *1000 to convert to km 
  dplyr::select(radius)

# transforming the coordinates into a spatial object
...
#Help functions 
```{r functions to help }
#function to transform the latitude longitude projection of each point into the right UTM
...
#transform into the right projection
...
# create buffers based on the list of spatial objects
...
# Retransform everything into a latitude longitude coordinate system
buf_latlong <- lapply(buf_proj, st_transform, 4326)

#recombining the whole thing
...
``` {r load raster}
map_raster <- terra::rast(paste0(here::here("data-raw","geo-raw"),"/wdpa_bin_prj.tif"))
...
extracted_values_mean_wdpa <- raster::extract(map_raster, buffer_new_projection,fun=mean, na.rm=TRUE)

extracted_values_mean_wdpa

saveRDS(extracted_values_mean_wdpa,paste0(data.output,"/data-proc/extracted_values_mean_wdpa",Sys.Date(),".rds"))
```

# Plot buffers with countries

``` {r plot buffers}
combined_buffer_wdpa <- cbind(buffer_new_projection,extracted_values_mean_wdpa)
mapview(combined_buffer_wdpa, zcol="wdpa_bin_prj")

dataframe_combined_buffer_wdpa <- study_area_size_data %>%
  na.omit() %>%
  cbind(combined_buffer_wdpa)

saveRDS(dataframe_combined_buffer_wdpa,paste0(data.output,"/data-proc/dataframe_combined_buffer_wdpa",Sys.Date(),".rds"))
```
