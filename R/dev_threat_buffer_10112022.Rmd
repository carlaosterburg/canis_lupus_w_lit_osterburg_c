...
``` {r load raster}
map_raster <- terra::rast(paste0(here::here("data-raw","geo-raw"),"/dev_threat_prj_maskf.tif"))
...
extracted_values_mean_dev_threat <- raster::extract(map_raster, buffer_new_projection,fun=mean, na.rm=TRUE)

extracted_values_mean_dev_threat

saveRDS(extracted_values_mean_dev_threat,paste0(data.output,"/data-proc/extracted_values_mean_dev_threat",Sys.Date(),".rds"))
```

# Plot buffers with countries

``` {r plot buffers}
combined_buffer_dev_threat <- cbind(buffer_new_projection,extracted_values_mean_dev_threat)
mapview(combined_buffer_dev_threat, zcol="dev_threat_prj_maskf")

dataframe_combined_buffer_dev_threat <- study_area_size_data %>%
  na.omit() %>%
  cbind(combined_buffer_dev_threat)

saveRDS(dataframe_combined_buffer_dev_threat,paste0(data.output,"/data-proc/dataframe_combined_buffer_dev_threat",Sys.Date(),".rds"))
```